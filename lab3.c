...
int main(int argc, char* argv[]){
    ...
    fileOK = processCharacters(&wordListPtr,numWordsPtr,argv[1]);
    if(!fileOK){
    ...
    while(ch != EOF) {
        if(isalpha(ch)){
            ...
            if(n>0){ /*?*/
                ...
                newWord.word = (char*)malloc((wordLength+1)*sizeof(char));
